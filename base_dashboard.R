...
f2c_porfolios$`Portfolio - Proportional Buildout 1.31.2024` <- 
  f2c_porfolios$`Portfolio - Proportional Buildout 1.31.2024` %>% 
  select(-`Input Category`) # Removing unnecessary column

# From Excel Files

for (portfolio_selector in seq_along(portfolio_names)){
  ...
  avoided_non_electric %>% 
    ...
    write_csv("output/non_electric_avoided_emissions_full.csv", append = portfolio_selector != 1)
  
  avoided_electric %>% 
    mutate(portfolio = portfolio_names[portfolio_selector]) %>%  
    write_csv("output/electric_avoided_emissions_full.csv", append = portfolio_selector != 1)
  ...
  water_use %>% mutate(portfolio = portfolio_names[portfolio_selector]) %>% 
    write_csv("output/water_commodity_base_full.csv", append = portfolio_selector != 1)
  ...
  land_use %>% mutate(portfolio = portfolio_names[portfolio_selector]) %>% 
    write_csv("output/land_commodity_base_full.csv", append = portfolio_selector != 1)
  ...
  jobs_use %>% mutate(portfolio = portfolio_names[portfolio_selector]) %>% 
    write_csv("output/jobs_commodity_base_full.csv", append = portfolio_selector != 1)
